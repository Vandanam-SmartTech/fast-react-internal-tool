...
export default function ListOfUsers() {
 ...
 const loadUsers = async (page: number) => {
  try {
    ...
    const data = await fetchRepresentativesPaginated(page, selectedRole);
...
<div className="relative w-72">
  <select
    ...
    className="w-full appearance-none border border-gray-300 rounded px-3 py-2 pr-8 focus:outline-none"
  >
  ...
  {/* Dropdown Icon when no role is selected */}
  {!selectedRole && (
  ...
  {/* Clear X Button when a role is selected */}
  {selectedRole && (
    <button
      type="button"
      onClick={() => {
...
{loading ? (
  <div className="flex justify-center items-center gap-2 py-10 text-lg font-medium text-gray-600">
