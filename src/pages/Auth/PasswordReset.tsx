...
import { resolvePasswordResetUser, sendPasswordResetOtp } from '../../services/jwtService';
...
const extractErrorMessage = (err: any): string => {
  ...
  return data?.message || data?.error || err?.message || 'Failed to send OTP.';
};
...
const PasswordReset: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const resolvedUser = await resolvePasswordResetUser(rawIdentifier);
      await sendPasswordResetOtp({
        identifier: rawIdentifier,
        channel: resolvedUser.channel,
        clientRequestId: `forgot-password-${Date.now()}`,
      });

      const expiryTime = Date.now() + 3 * 60 * 1000;
      const resendTime = Date.now() + 60 * 1000;
      const successMessage = `OTP sent via ${resolvedUser.channel === 'SMS' ? 'SMS' : 'email'}.`;

      setMessage(successMessage);
      toast.success(successMessage, { autoClose: 1000, hideProgressBar: true });

      navigate('/verification', {
        state: {
          identifier: rawIdentifier,
          resetEmail: resolvedUser.emailAddress,
          channel: resolvedUser.channel,
          msg: successMessage,
          expiryTime,
          resendTime,
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-full bg-cover bg-center flex justify-center items-center px-4" style={{ backgroundImage: `url(${bgImage})` }}>
      <div className="w-full max-w-xs sm:max-w-sm md:max-w-md p-5 sm:p-6 bg-white bg-opacity-90 rounded-lg shadow-lg">
        ...
        {message && !loading && <div className="bg-green-100 text-green-700 border border-green-200 rounded-lg p-2 sm:p-3 mb-4 text-center">{message}</div>}
        {error && !loading && <div className="bg-red-100 text-red-700 border border-red-200 rounded-lg p-2 sm:p-3 mb-4 text-center">{error}</div>}
        ...
        <form onSubmit={handleSubmit}>
          <div>
            ...
            <p className="text-sm text-gray-600 mb-2">Enter your email, mobile number, or username. We&apos;ll send a one-time password only if that account is already registered.</p>
          </div>
          <div>
            <label htmlFor="email" className="block text-sm font-medium text-gray-600 mb-3">Email, Mobile, or Username</label>
            <input
              id="email"
              type="text"
              value={emailInput}
          ...
          <button type="submit" className={`w-full px-2 py-2 text-white rounded-lg font-semibold transition ${loading ? 'bg-blue-300 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`} disabled={loading}>
