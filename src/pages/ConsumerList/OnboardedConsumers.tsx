...
import { fetchOnboardedConsumers, searchOnboardedConsumers, fetchVillages } from "../../services/customerRequisitionService";
...
const OnboardedConsumers: React.FC = () => {
  ...
  const { userClaims, loading: contextLoading } = useUser();
  ...
  const resetAndLoad = async () => {
    ...
    await loadOnboardedConsumers(0);
  };
  ...
  const handleNextPage = async () => {
    ...
    if (!isSearching) {
      if (
        ...
        await loadOnboardedConsumers(backendPage + 1);
      }
    }
  ...
  const loadOnboardedConsumers = async (pageNumber: number) => {
    if (!isInitialized) {
      return;
    }
    ...
    try {
      ...
      const data = await fetchOnboardedConsumers(pageNumber,
        BATCH_SIZE,
        params);
  ...
  useEffect(() => {
    ...
    searchTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        ...
        const data = await searchOnboardedConsumers(
          trimmed,
          0,                 // page number
          BATCH_SIZE,        // limit
          params
